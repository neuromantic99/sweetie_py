...
def loadFolderToArray(folderpath, channels = 'all', dtype = float, source = '100'):
    ...
    channel_1_channel = loadContinuous(os.path.join(folderpath, filelist[0]), dtype)['header']['channel']
    ...
    channel_array = []
    channel_array.append(channel_1_channel)
    #np.zeros(n_channels, 'U25')
    #channel_array.append(channel_1_channel)
    

    for i, f in enumerate(filelist[1:]):
            data_array[:, i + 1] = loadContinuous(os.path.join(folderpath, f), dtype)['data']
            channel_array.append(loadContinuous(os.path.join(folderpath, f), dtype)['header']['channel'])
            numFiles += 1

    print(''.join(('Avg. Load Time: ', str((time.time() - t0)/numFiles),' sec')))
    print(''.join(('Total Load Time: ', str((time.time() - t0)),' sec')))

    return data_array, channel_array

def loadContinuous(filepath, dtype = float):

    assert dtype in (float, np.int16),       'Invalid data type specified for loadContinous, valid types are float and np.int16'

    print("Loading continuous data from %s" %filepath)
...
def pack_2(folderpath, filename = 'openephys.dat', source='100', channels = 'all', dref = None):
    ...
    [data_array, channel_array] = loadFolderToArray(folderpath, channels, np.int16, source)
...
fnameWhisk =  '2017-07-20_16-38-05'
fnameOpto = '2017-07-20_17-25-49'


[data_array1, channel_array1] = pack_2(fpath + fnameWhisk, source = '108',filename = fnameWhisk  + 'less5' +  '.dat' , channels = singleShankOrder, dref = 'ave')

[data_array2, channel_array2] = pack_2(fpath + fnameOpto, source = '108',filename = fnameOpto  + 'less5' +  '.dat' , channels = singleShankOrder, dref = 'ave')



both = np.concatenate((data_array1, data_array2))




print('Packing data to file: ' + 'joined')
both.tofile(os.path.join(fpath,'joinedPV.dat'))
